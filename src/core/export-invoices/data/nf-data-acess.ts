...
export class NfRepository {
    ...
    async findNotasByDataEmissao(dataInicio: string, dataFim: string): Promise<CadNf[]> {
        return new Promise(async (resolve, reject) => {
            const sql = `
                SELECT 
                    nf.*,
                ...
                ORDER BY nf.DATA_EMISSAO, nf.CODIGO
            `;
            conn.query(sql, [dataInicio, dataFim], (err, result: CadNf[]) => {
                if (err) {
                    reject(err);
                } else {
                    resolve(result);
    ...
    async findNotasByCodigos(codigos: number[]): Promise<CadNf[]> {
        return new Promise(async (resolve, reject) => {
            if (codigos.length === 0) return resolve([]);
            ...
            const sql = `
                SELECT 
                    ...
                    CAST(nf.DADOS_ADICIONAIS AS CHAR(1000) CHARACTER SET latin1) as DADOS_ADICIONAIS
                FROM ${db_vendas}.cad_nf nf
                WHERE nf.CODIGO IN (${placeholders})
                ORDER BY nf.DATA_EMISSAO, nf.CODIGO
            `;
            conn.query(sql, codigos, (err, result: CadNf[]) => {
                if (err) {
                    reject(err);
                } else {
                    resolve(result);
    ...
    async findResumoNotas(dataInicio: string, dataFim: string): Promise<any[]> {
        return new Promise(async (resolve, reject) => {
            const sql = `
                SELECT 
                    nf.CODIGO,
                ...
                ORDER BY nf.DATA_EMISSAO, nf.CODIGO
            `;
            conn.query(sql, [dataInicio, dataFim], (err, result) => {
                if (err) {
                    reject(err);
                } else {
                    resolve(result);
    ...
    async findTransacao(codigoTransacao: number): Promise<{ NATUREZA: string } | null> {
        return new Promise(async (resolve, reject) => {
            const sql = `
                SELECT NATUREZA
                FROM ${db_vendas}.transacoes
                WHERE CODIGO = ?
            `;
            conn.query(sql, [codigoTransacao], (err, result: any[]) => {
                if (err) {
                    reject(err);
                } else {
